...
class Database:
    ...
    def add_new_task(self, assigned_user: int, title: str, description: str, deadline: datetime.datetime,
    ...
    # log for histories
    def add_log_new_assigned_user(self, task_id: int, updated_time: datetime.datetime, change_type: str,
                                  new_assigned_user: int):
        '''
        add a new log for changing assigned_user of a task
        :param task_id:
        :param updated_time: from frontend
        :param change_type: 'A'
        :param new_assigned_user:
        :return: {}
        '''
        log_data = [str(task_id), str(updated_time),
                     change_type, str(new_assigned_user)]
        insert_query = "insert into Histories (task_id, updated_time, change_type, new_assigned_user) values (%s, %s, %s, %s)"
        with self.db.cursor() as cursor:
    ...
    def add_log_new_task_status(self, task_id: int, updated_time: datetime.datetime, change_type: str,
                                new_task_status: str):
        '''
        add a new log for changing task_status of a task
        :param task_id:
        :param updated_time: from frontend
        :param change_type: 'S'
        :param new_task_status: 'Not Started','In Progress','Blocked','Completed'
        :return: {}
        '''
        log_data = [str(task_id), str(updated_time),
                     change_type, new_task_status]
        insert_query = "insert into Histories (task_id, updated_time, change_type, new_task_status) values (%s, %s, %s, %s)"
        with self.db.cursor() as cursor:
            cursor.execute(insert_query, log_data)
    ...
    def get_task_histories(self, task_id: int):
        ...
        :param task_id:
        :return: [{"task_id": task_id, "updated_time": updated_time, "change_type": change_type, "new_assigned_user": new_assigned_user, "new_task_status": new_task_status}]
        :exeption: AccessError :if no such task_id
        '''
        histories = []
        ...
        if match_task > 0:
            ...
            for row in result:
                ...
                updated_time = row[1]
                change_type = row[2]
                new_assigned_user = row[3]
                new_task_status = row[4]

                history = {"task_id": task_id, "updated_time": updated_time, "change_type": change_type,
                           "new_assigned_user": new_assigned_user, "new_task_status": new_task_status}
    ...
    def update_request_status(self, sender_id: int, receiver_id: int, request_status: str):
        ...
        with self.db.cursor() as cursor:
            cursor.execute(
                query, (request_status, sender_id, receiver_id))
